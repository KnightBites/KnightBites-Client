  ...
  const getDishData = async () => {
    try {
      ...
      setDishData(json.map(dish => ({
        ...
        img: dish.foodimage, // TODO: find a way to fetch images from the database... force them to the right size. Database does not have dish URL's, add FoodImage to db and web-service so /dish endpoint will return foodimage URL field.
      }))); // add rating to dish
    } catch (err) {
      console.error(err);
    } finally {
